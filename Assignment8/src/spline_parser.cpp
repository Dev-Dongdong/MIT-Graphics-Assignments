...
void SplineParser::SaveBezier(ArgParser *args) {
  ...
  string newPath = args->output_bezier_file;
  newPath = "../output_txts/" + newPath;
...
void SplineParser::SaveBSpline(ArgParser *args) {
  ...
  string newPath = args->output_bspline_file;
  newPath = "../output_txts/" + newPath;
  FILE *file = fopen(newPath.c_str(),"w");
  assert (file != NULL);
  fprintf (file, "num_splines %d\n", getNumSplines());
...
void SplineParser::SaveTriangles(ArgParser *args) {
  ...
  string newPath = args->output_file;
  newPath = "../output_objs/" + newPath;
  FILE *file = fopen(newPath.c_str(),"w");
  assert (file != NULL);
