...
unsigned char ReadByte(FILE *file) {  //将文件以字符形式读出
  unsigned char b;
...
void WriteByte(FILE *file, unsigned char b) { //以字符写入文件
  int success = fwrite((void*)&b,sizeof(unsigned char),1,file);
  assert (success == 1);
}

unsigned char ClampColorComponent(float c) {  //将c从[0,1]线性插值至[0,255]
  int tmp = c * 255;
  if (tmp < 0) tmp = 0;
  if (tmp > 255) tmp = 255;
...
// ====================================================================
// ====================================================================
//返回img1中每个像素img2中对应像素颜色之差组成的image
