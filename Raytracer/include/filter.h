 ...
 * @LastEditTime: 2021-03-11 16:33:53
...
public:
    ...
    float getWeight(float x, float y) { if( abs(x) < radius && abs(y) < radius ) return 1;
                                        else return 0; }
...
public:
    ...
    float getWeight(float x, float y) { float dis = sqrt(x*x + y*y); return (dis > radius) ? 0 : 1 - dis / radius; }
    int getSupportRadius() { float check = radius - 0.5f; return (abs(check - (int)check) > EPSILON) ? (int)check+1 : (int)check; }
};
