...
void SceneParser::parseMaterials() {
  ...
  while (num_materials > count) {
    ...
    } else if (!strcmp(token, "SSSMaterial")) {
      materials[count] = parseSSSMaterial();
    } else {
...
Material* SceneParser::parseSSSMaterial() {
  char token[MAX_PARSER_TOKEN_LENGTH];
  // Phong Material Variables
  Vec3f diffuseColor(1,1,1);
  Vec3f specularColor(0,0,0);
  float exponent = 1;
  Vec3f reflectiveColor(0,0,0);
  Vec3f transparentColor(0,0,0);
  float indexOfRefraction = 1;
  // SSS Material Variables
  float wrap = 10;
  float C = 5;
  float L2NDistortion = 5; 
  float power = 5, scale = 1; 
  float attenuation = 1.f; 
  Vec3f ambient(0.36,0.9,0.25);
  getToken(token); assert (!strcmp(token, "{"));
  while (1) {
    getToken(token); 
    if (!strcmp(token, "diffuseColor")) {
      ...
      indexOfRefraction = readFloat();
    } else if (!strcmp(token, "wrap")) {
      wrap = readFloat();
    } else if (!strcmp(token, "C")) {
      C = readFloat();
    } else if (!strcmp(token, "L2NDistortion")) {
      L2NDistortion = readFloat();
    } else if (!strcmp(token, "power")) {
      power = readFloat();
    } else if (!strcmp(token, "scale")) {
      scale = readFloat();
    } else if (!strcmp(token, "attenuation")) {
  ...
  Material *answer = new SSSMaterial(diffuseColor,specularColor,exponent,
				       reflectiveColor,transparentColor,
				       indexOfRefraction,wrap,C,L2NDistortion,power,scale,attenuation,ambient);
  return answer;
}
