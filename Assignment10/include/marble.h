 ...
 * @LastEditTime: 2021-10-17 21:15:11
...
public:
    ...
    virtual Vec3f Shade(const Ray &ray, const Hit &hit, const Vec3f &dirToLight, 
     const Vec3f &lightColor, const float &distance, RayTracer* raytracer) {
        ...
        M = (M + 1) / 2;
        return mat1->Shade(ray,hit,dirToLight,lightColor,distance,raytracer) * (M) + mat2->Shade(ray,hit,dirToLight,lightColor,distance,raytracer) * (1 - M);
    }
};
