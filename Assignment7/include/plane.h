 ...
 * @LastEditTime: 2021-02-21 20:14:09
...
public:
    Plane(const Vec3f &n, const float &_offset, Material* mat){normal = n; offset = _offset; m = mat; box = NULL;}
    ~Plane(){}
...
bool Plane::intersect(const Ray &r, Hit &h, float tmin){
    ...
    if(abs(offset - OdotN) < FLT_EPSILON){  // the ray origin is in this plane.[Intersected]
        if(tmin > 0 || abs(tmin) < EPSILON)return false;    //tmin <= 0 and this must return false
        h.set(0,m,normal,r);
        return true;
    }
